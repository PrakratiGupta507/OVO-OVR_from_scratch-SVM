...
Original file is located at
    https://colab.research.google.com/drive/1-wG-GZP7pZissSruXxEZ1Oem8TU2ovUC
"""
...
for a in range (1 , 6):
    ...
    for i in C:
      for j in gamma:
        svclassifier = SVC(decision_function_shape=custom_ovr,kernel='rbf',C=i,gamma=j)
...
def custom_ovr(df):
  target = 'label'
  unique_labels = np.unique(np.array(df[target]))
  models = []
...
for i in range (1 , 6):
  ...
  y=test_fold['label']
  X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.20)
  svclassifier = SVC(decision_function_shape=custom_ovr,kernel='rbf',C=10,gamma=0.001)
  svclassifier.fit(X_train, y_train)
  y_pred = svclassifier.predict(X_test)
